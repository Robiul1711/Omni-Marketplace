...
const PrivacyPolicy = () => {
  const { data: privacyCmsData } = useClient({
    queryKey: ["privacyPolicyCms"],
    url: "/cms-pages/privacy-policy",
  });

  const pageTitle = privacyCmsData?.data?.title || "Privacy Policy";
  const lastUpdated = privacyCmsData?.data?.content?.last_updated || "2026-04-16";
  const description = privacyCmsData?.data?.content?.description;
