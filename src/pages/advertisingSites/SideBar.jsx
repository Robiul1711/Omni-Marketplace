...
const SideBar = ({ sidebar, open, setOpen }) => {
  ...
  // Map icon names to components if they are strings, or use them directly if they are already JSX
  const renderIcon = (icon) => {
    if (typeof icon === "string") {
      switch (icon) {
        case "Dashboard": return <MdOutlineDashboard size={22} />;
        case "My Campaigns": return <FiBox size={22} />;
        case "Saved Placements": return <FiHeart size={22} />;
        case "Payment History": return <FiDollarSign size={22} />;
        case "Settings": return <FiSettings size={22} />;
        default: return <MdOutlineDashboard size={22} />;
      }
    }
    return icon;
  };

  // Override icons based on text content for better consistency with the design
  const getIcon = (item) => {
    switch (item.text) {
      case "Dashboard": return <MdOutlineDashboard size={22} />;
      case "My Campaigns": return <FiBox size={22} />;
      ...
      >
        ...
        <div className="p-8 flex items-center gap-3">
          <Link to="/" className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#3366FF] rounded-xl flex items-center justify-center shadow-lg shadow-blue-200">
              <span className="text-white font-bold text-xl">O</span>
            </div>
            <span className="text-[#1A1D1F] font-bold text-xl tracking-tight">
              Omni <span className="text-gray-400">Marketplace</span>
            </span>
          </Link>
        </div>
            ...
            {sidebar?.map((item, index) => {
              ...
              return !item?.sublink ? (
                ...
                <div className="flex flex-col gap-1" key={index}>
                  <div
                    className={`flex items-center justify-between px-4 py-3.5 cursor-pointer rounded-xl transition-all duration-200 ${
                      active
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <span className={active ? "text-[#3366FF]" : ""}>{getIcon(item)}</span>
                      <span className="font-semibold text-[15px]">{item?.text}</span>
                    </div>
                    <MdKeyboardArrowDown
                      size={20}
                    ...
                    <div className="ml-11 flex flex-col gap-1 mt-1 border-l-2 border-gray-100 pl-4 animate-in slide-in-from-top-2 duration-300">
                      {item?.sublink?.map((sub, idx) => (
                        <Link
                          ...
                          to={sub?.path}
                          onClick={() => setOpen(false)}
                          className={`py-2 text-[14px] font-medium transition-colors duration-200 ${
                            isActive(sub.path)
                              ? "text-[#3366FF]"
